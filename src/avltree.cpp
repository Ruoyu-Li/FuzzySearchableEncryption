								 ...
								 avl_node* parent, void* data )
{
    ...
    avl_node* an = (avl_node*)calloc( 1, sizeof(avl_node) + handle->data_size );
	if( an == 0 ) return 0;
	if( parent )
	{
		if( handle->avl_cmp(data, parent->data) < 0 )
			parent->left  = an;
		else
			parent->right = an;
		an->parent = parent;
...
static int avl_search( avl_handle* handle, avl_node** node, void* key )
{
	...
	{
		history = an;
		compare = handle->avl_cmp( key, an->data );
		if( compare < 0 )
			an = an->left;
		else if( compare > 0 )
			an = an->right;
...
{
    avl_handle* ah = (avl_handle*)calloc( 1, sizeof(avl_handle) );
	if( ah == 0 ) return 0;
